...
async function getLDEM(xml_collection, [lat_prefix, long_suffix], imageBuffer){
    ...
    for( let lat_name_portion of lat_prefix.reverse()){
        ...
        /**
         * each latitude has 8 cooresponding sub-tiles assoicated with it.
         * l1, l2... contains the data assoicated with sub-tile (starting from left -- > right)
         */
        for(let rows = 0; rows < MAX_ROWS; rows++){
            ...
            let chunk3 = ldem_raw_data.slice(from_pos, to_pos);
            l3.push(chunk3);
            from_pos += MAX_COLS__IN_BYTES;
            to_pos += MAX_COLS__IN_BYTES;

            let chunk4 = ldem_raw_data.slice(from_pos, to_pos);
            l4.push(chunk4);
            from_pos += MAX_COLS__IN_BYTES;
            to_pos += MAX_COLS__IN_BYTES;

            let chunk5 = ldem_raw_data.slice(from_pos, to_pos);
            l5.push(chunk5);
            from_pos += MAX_COLS__IN_BYTES;
            to_pos += MAX_COLS__IN_BYTES;

            let chunk6 = ldem_raw_data.slice(from_pos, to_pos);
            l6.push(chunk6);
            from_pos += MAX_COLS__IN_BYTES;
            to_pos += MAX_COLS__IN_BYTES;

            let chunk7 = ldem_raw_data.slice(from_pos, to_pos);
            l7.push(chunk7);
            from_pos += MAX_COLS__IN_BYTES;
            to_pos += MAX_COLS__IN_BYTES;

            let chunk8 = ldem_raw_data.slice(from_pos, to_pos);
            l8.push(chunk8);
            from_pos += MAX_COLS__IN_BYTES;
            to_pos += MAX_COLS__IN_BYTES;
        }
        console.log("Processed 8 tiles");
        ...
        idx++;
        n = await writeData(xml_collection, lat_name_portion + long_suffix[2], l3 );

        idx++;
        n = await writeData(xml_collection, lat_name_portion + long_suffix[3], l4 );

        idx++;
        n = await writeData(xml_collection, lat_name_portion + long_suffix[4], l5 );

        idx++;
        n = await writeData(xml_collection, lat_name_portion + long_suffix[5], l6);

        idx++;
        n = await writeData(xml_collection, lat_name_portion + long_suffix[6], l7 );

        idx++;
        n = await writeData(xml_collection, lat_name_portion + long_suffix[7], l8 );
...
self.onmessage = async function(params){
    ...
    try{
        const origin = "https://pds-geosciences.wustl.edu/lro/lro-l-lola-3-rdr-v1/lrolol_1xxx/data/lola_gdr/cylindrical/img/";
        //const origin = "https://127.0.0.1:5500/webPTT/img/LOLA_data/ldem512/";

        // fetch file and post messages about prgress to parent "load_resource_main's main_wwt.onmessage"
        const filename = params.data.meta.filename + ".img";
        let imageBuffer = new Uint8Array(2123366400);
        let offset = 0;
    ...
    try{
        ...
        //self.postMessage({files : files, num_files: files.length});
        self.close();
    }
    catch(err){
