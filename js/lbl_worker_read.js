...
self.onmessage = async function (params) {
  ...
  try {
    const sub_dir = await root.getDirectoryHandle(directory_name, {
      create: false,
    });

    let img_data_array = [];
    let img_lat_lon_array = [];
    for await (let fn of tile_names) {
      // fn = fn +".bin";
      fn = fn;
    ...
    for (fn of tile_names) {
      ...
      let is_south = /s/.test(fn_buff); // MOLA spans north and south
      ...
      if (is_north && is_south) {
        // special case for MOLA, negative on parts[1] only
        parts[1] = "-" + parts[1];
      } else if (is_north === false) {
        // add minus sing
        parts[0] = "-" + parts[0];
        parts[1] = "-" + parts[1];
      }
    ...
    for (let count = 0; count < num_tiles; count++) {
      ...
      self.postMessage(
        { success: true, tile_data: buf, lat_lon: lats_lons, count: count },
        [buf]
      );
    }
  } catch (err) {
